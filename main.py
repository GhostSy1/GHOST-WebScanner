...
async def run_scan(target):
    ...
    with Progress(
        ...
        task1 = progress.add_task(description="Performing Reconnaissance...", total=None)
        recon_results = await recon.run()
        
        task2 = progress.add_task(description="Scanning for Vulnerabilities...", total=None)
        vuln_results = await vuln.run()
        
        task3 = progress.add_task(description="Generating Report...", total=None)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ghost-SY1 Advanced Web Security Suite")
    parser.add_argument("-u", "--url", help="Target URL (e.g., https://example.com)", required=False)
